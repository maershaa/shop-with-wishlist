...
// // Not found
/* function foundCelebrity(people) {
  const candidates = people.filter(el => el.know.length === 0);
  ...
  const narciss = candidates[0];

  const isKnownByAll = people.every(person => {
    if (person.name === narciss.name) {
      return true;
    } else {
      return person.know.includes(narciss.name);
    }
  });

  return isKnownByAll ? narciss.name : 'Not found';
}
...
console.log(foundCelebrity(people1)); // Not found
console.log(foundCelebrity(people2)); // Not found
console.log(foundCelebrity(people3)); // Jhon */
// --------------------------------------------------------------------------------------------------------//
...
films.forEach(el => {
  el.genreNames = [];
  for (let i = 0; i < genresInfo.genres.length; i++) {
    if (el.genreIDs.includes(genresInfo.genres[i].id)) {
      el.genreNames.push(genresInfo.genres[i].name);
    }
  }

  if (el.genreNames.length > 2) {
    let x = el.genreNames.length - 2;
    el.genreNames.splice(2, x, 'Others');
  }
  console.log(el.title, ':', el.genreNames);
});
...
class User {
  // Приватные свойства объявляются через #
  #name;
  #surname;
...
class Student extends User {
  constructor(name, surname, year) {
    super(name, surname); // Передаем данные в родительский конструктор
    this.year = year;
  }
  getCourse() {
    const date = new Date();
    ...
    if (course < 5) {
      console.log(`студент ${course} курсу `);
    } else if (course > 5) {
      console.log(`студент завершив навчання`);
    } else {
      console.log(`студент ${course} курсу та являєтсья випускником`);
    }
  }
}
...
student.getCourse(); //поверне 6 курс

const student2 = new Student('Valeria', 'Maers', 2023);

student2.getFullName(); //поверне 'Valeria Maers'
student2.getCourse(); //поверне 2
